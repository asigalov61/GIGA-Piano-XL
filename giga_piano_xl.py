...
Original file is located at
    https://colab.research.google.com/drive/1SwrZfOhGbeBZsp3Yewy4C68qoZ__u_gR

# GIGA-Piano XL (ver. 1.0)
...
full_path_to_custom_MIDI = "/content/GIGA-Piano-XL/GIGA-Piano-XL-Seed-1.mid" #@param {type:"string"}
...
for i in tqdm(range(number_of_continuation_blocks)):
  ...
  rand_seq = model.generate(torch.Tensor(out1[-number_of_memory_tokens:]), 
...
for i in tqdm(range(number_of_continuation_chunks)):
  ...
  rand_seq = model.generate_batches(torch.Tensor(out1[-number_of_memory_tokens:]), 
...
control_type = "Time" #@param ["Time", "Time-Duration", "Time-Duration-Pitch", "Pitch"]
...
for i in tqdm(range(number_of_prime_notes+2, len(melody_chords)-1)):
  ...
  rand_seq = model.generate(torch.Tensor(out1[-(number_of_memory_tokens-ctrl):]), 
...
for i in tqdm(range(number_of_prime_notes+1, len(melody))):
  ...
  for j in range(nnpc):
    ...
    rand_seq = model.generate(torch.Tensor(inp[-(number_of_memory_tokens-1):]), 
